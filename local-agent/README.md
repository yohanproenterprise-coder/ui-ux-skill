...
- **✦ Agrandir ×2 / ×4 avec l'IA** (onglet Réglages) : recrée les détails pour une photo plus grande et plus nette, avec un rendu naturel (grain de peau conservé). Modèle inclus (Real-ESRGAN), il suffit d'installer une fois le petit composant proposé (≈ 20 Mo). Compte quelques secondes à une minute selon la taille ;
